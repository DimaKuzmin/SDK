...
void VIPM_MultiTH::VIPM_Init(int th)
{
	//.	OutputDebugString("VIPM_INIT-------------------\n");
	//R_ASSERT2(0 == g_mt_pObject, "VIPM already in use!");

	//if (g_mt_pObject)
	//	Msg("VIPM Already in use!!!!");
	g_mt_pObject = xr_new<Object>();
	g_mt_pResult = xr_new<VIPM_Result>();
	g_mt_pObject->iNumCollapses = 0;
	g_mt_pObject->iCurSlidingWindowLevel = 0;
}

void VIPM_MultiTH::VIPM_AppendVertex(const Fvector3& p, const Fvector2& uv)
{
	MeshPt* pt = xr_new<MeshPt>(&g_mt_pObject->CurPtRoot);
	g_mt_ppTempPts.push_back(pt);
	pt->mypt.vPos = p;
	pt->mypt.fU = uv.x;
	pt->mypt.fV = uv.y;
	pt->mypt.dwIndex = g_mt_ppTempPts.size() - 1;
}

void VIPM_MultiTH::VIPM_AppendFace(u16 v0, u16 v1, u16 v2)
{
	xr_new<MeshTri>(g_mt_ppTempPts[v0], g_mt_ppTempPts[v1], g_mt_ppTempPts[v2], &g_mt_pObject->CurTriRoot, &g_mt_pObject->CurEdgeRoot);
}

VIPM_Result* VIPM_MultiTH::VIPM_Convert(u32 max_sliding_window, float error_tolerance, u32 optimize_vertex_order)
{
	g_mt_pObject->Initialize();
	if (!g_mt_pObject->Valid())return NULL;

	CalcALL(g_mt_pObject, max_sliding_window, error_tolerance);	 //CHECK THIS
	
	if (CalculateSW(g_mt_pObject, g_mt_pResult, optimize_vertex_order))
	{
		return g_mt_pResult;
	}
	else					return NULL;
...
void VIPM_MultiTH::VIPM_Destroy()
{
	xr_delete(g_mt_pResult);
	xr_delete(g_mt_pObject);
	g_mt_ppTempPts.resize(0);
}

ICF void VIPM_MultiTH::CalcALL(Object* m_pObject, u32 max_sliding_window, float m_fSlidingWindowErrorTolerance)
{
	m_pObject->BinEdgeCollapse();
	while (true)
	{
