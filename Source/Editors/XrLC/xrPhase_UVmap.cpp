...
void CBuild::xrPhase_UVmap()
{
	...
	for (int SP = 0; SP<int(g_XSplit.size()); SP++)
	{
		...
		if (LastSP != SP && LastSP < LastXSplit)
		{
			...
			Status("SP[%d] all[%d] T[%f]", SP, g_XSplit.size(), timer_gl.GetElapsed_sec());
		...
		while (TRUE)
		{
			...
			if (msF)
			{
				...
				//Status("SP[%d], face[%d]/[%d], all[%d] T[%f]", SP, id_face, g_XSplit[SP]->size() - remove_count, g_XSplit.size(), timer_gl.GetElapsed_sec());
			}
			else
			{
