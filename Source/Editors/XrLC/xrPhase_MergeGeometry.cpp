...
auto Validate = [](u32& CurrentProcessedID, u32& VecIndex, data_material& cmaterial_subdiv, xr_vector<data_material>& data_vector, Fbox bb_base, Fbox bb_base_orig)
	{
		...
		{
			concurrency::parallel_for(size_t(0), size_t(data_vector.size()), [&](size_t Index)
			{
				FunctionItem(Index);
			});
		}
...
{
	concurrency::concurrent_unordered_map<int, xr_vector<data_material>> thread_faces;
	...
	for (auto& MAP : thread_faces)
	{
		...
		for (auto& grid : grid_map)
		{
			...
			{
				clMsg("MP(%u/%u) SP(%u) GRID[%u](%u/%u) SP(%u)", IndexMap, thread_faces.size(), MAP.second.size(), GridMAX, IDX_GRID, grid_map.size(), grid.second.size());
		...
		{
			clMsg("MP(%u/%u) SP(%u)", IndexMap, thread_faces.size(), MAP.second.size());
...
void xrPhase_MergeGeometry_Tbb()
{
	...
	int min = size_t(1);
	int max = g_XSplit.size() / tbb::task_arena().max_concurrency() / 10;;
