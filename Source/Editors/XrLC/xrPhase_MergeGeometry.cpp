...
ICF void FindWhileMergeNeed(bool USE_MT, int id)
{
	...
	for (;;)
	{
		...
		{
			//Status("IDX_Works[%d] MERGE: %d TIMESEC: [%d], TRY_FIND: %d", thread_faces.size(), faces_vect.size(), (u32)tGlobalMerge.GetElapsed_sec(), IDwhile);
			IDwhile++;
...
void CBuild::xrPhase_MergeGeometry()
{
	...
	else
	{
		...
		for (u32 split = 0; split < g_XSplit.size(); split++)
		{
			...
			Status("Merge %d/%d, time: %.0f", split, g_XSplit.size(), t.GetElapsed_sec());
		}
