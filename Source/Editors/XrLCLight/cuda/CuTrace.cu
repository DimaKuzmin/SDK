...
__device__ void CalculatePoint(Hardware_Raytask& Task, Hardware_Lighting& L, unsigned int TaskID)
{
	...
	switch (L.type)
	{
		...
		{	
			...
			// sqrt плохо работал 
			float R;
			Ldir.Subtract(LightPosition, P);
			Ldir.GetRangeNormalized(sqD, R);
			...
			float invAtt = 1.0f / ( L.attenuation0 + L.attenuation1 * R + L.attenuation2 * sqD );
 			if (isSunOrHemi)
				att = scale * invAtt;
			else
				att =  scale * (invAtt - R * L.falloff);
		} break;
