...
{
	bool avx_test	= gCompilerMode.use_avx2;
	bool sse4		= CPU::ID.feature & _CPU_FEATURE_SSE4_2;

	string128 state;
	sprintf(state, "- Intilized Intel Embree %s - %s", RTC_VERSION_STRING, avx_test ? "avx" : sse4 ? "sse" : "default");
	Status(state);
